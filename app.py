...
# --- 1. 系統初始化 ---
st.set_page_config(page_title="AI 副官 v1.6c - 全量化自動版", layout="wide", page_icon="🛡️")
    ...
    try:
        ...
        try:
            ...
            if data['stat'] == 'OK':
                ...
                if foreign_data:
                    # 數值包含逗號，需移除 (第四欄是買賣超金額)
                    val_str = foreign_data[3].replace(',', '')
                    val_float = float(val_str)
                    # 證交所單位是元，轉為「億」方便閱讀
                    results['foreign_net'] = val_float / 100000000 
            else:
                results['foreign_net'] = 0
        except:
            results['foreign_net'] = 0 # 抓取失敗預設為 0
        ...
        if fred_key:
            ...
            try:
                ...
                results['cpi_ok'] = True; results['rate_low'] = True
...
with st.sidebar:
    ...
    if st.button("🔄 刷新全自動情報"):
        with st.spinner('連線證交所、FRED 與全球市場中...'):
            st.session_state['auto_m'] = fetch_auto_macro(fred_key)
    
    auto = st.session_state.get('auto_m', {})
    
    with st.expander("🌍 v1.6c 全量化指標 (12項)", expanded=True):
        # 1. 自動抓取區
        st.caption("【自動偵蒐】")
        m1 = auto.get('twd_strong', True)
        st.checkbox(f"台幣匯率走強", value=m1, disabled=True)
        
        m2 = auto.get('sox_up', True)
        st.checkbox(f"費半指數上揚", value=m2, disabled=True)
        
        m3 = auto.get('light_pos', True)
        st.checkbox(f"景氣燈號: {auto.get('light_name','-')}", value=m3, disabled=True)
        
        val_foreign = auto.get('foreign_net', 0)
        m4 = val_foreign > 0
        st.checkbox(f"外資買賣超: {val_foreign:.1f}億", value=m4, disabled=True)
        
        val_sp = auto.get('sp500_bull', True)
        m5 = val_sp
        st.checkbox(f"S&P500 多頭排列", value=m5, disabled=True)

        m6 = auto.get('cpi_ok', True)
        m7 = auto.get('rate_low', True)
        
        # 2. 數值判定區 (原本手動輸入的，現在大部分自動抓了)
        st.markdown("---")
        st.caption("【閾值自動判定】")
        
        val_yield = auto.get('yield_val', 4.0)
        m8 = val_yield < 4.5
        st.write(f"10Y 美債: **{val_yield:.2f}%** {'✅' if m8 else '❌ (>4.5)'}")
        
        val_dxy = auto.get('dxy_val', 104.0)
        m9 = val_dxy < 105.0
        st.write(f"美元指數: **{val_dxy:.2f}** {'✅' if m9 else '❌ (>105)'}")
        
        val_vix = auto.get('vix_val', 15.0)
        m10 = val_vix < 20.0
        st.write(f"VIX 恐慌: **{val_vix:.2f}** {'✅' if m10 else '❌ (>20)'}")

        st.markdown("---")
        st.caption("【手動輸入 (每月更新)】")
        # 僅保留這兩個無法自動化且必須數值化的指標
        v_pmi = st.number_input("製造業 PMI", value=50.0, step=0.1)
        m11 = v_pmi > 50.0
        
        v_export = st.number_input("出口訂單年增(%)", value=5.0, step=0.1)
        m12 = v_export > 0
    ...
    st.markdown(f"### 戰略總分: :{score_color}[{score}]")
    ...
    for i, t in enumerate(tickers):
        with cols[i]:
            ...
            if err: st.error(err)
                ...
                st.caption(f"K值: {res['k']:.1f} | 週線MACD: {'多' if res['signal']!='STAY AWAY' else '空'}")
